...
class PythonDocxRenderer(mistune.Renderer):
    ...
    def table(self, header, body):
        ...
        cells = ["table.rows[%d].cells[%d].paragraphs[0]%s\n" % (i, j, self.table_memory.pop(0)[1:]) for i, j in
                 itertools.product(range(number_rows), range(number_cols))]
        return '\n'.join(["table = document.add_table(rows=%d, cols=%d, style = 'BasicUserTable')" % (
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate Docx reports using a Docx reference template and Markdown files')
